...
class GoalsSetupBloc extends Bloc<GoalsSetupEvent, GoalsSetupState> {
  ...
  void _onGoalSelected(GoalSelected event, Emitter<GoalsSetupState> emit) {
    if (state is GoalsSetupLoaded) {
      ...
      final updatedGoals = currentState.goals.copyWith(
        ...
        targetCalories: (currentState.goals.targetCalories != null &&
                currentState.goals.targetCalories! >= 800 &&
                currentState.goals.targetCalories! <= 5000)
  ...
  void _onTargetWeightChanged(
      TargetWeightChanged event, Emitter<GoalsSetupState> emit) {
    if (state is GoalsSetupLoaded) {
      final currentState = state as GoalsSetupLoaded;
      final updatedGoals = currentState.goals.copyWith(
  ...
  void _onSaveGoals(SaveGoals event, Emitter<GoalsSetupState> emit) async {
    developer.log('🟤 GoalsSetupBloc: SaveGoals event received',
        name: 'GoalsSetupBloc');

    if (state is GoalsSetupLoaded) {
      final currentState = state as GoalsSetupLoaded;
      developer.log('🟤 GoalsSetupBloc: Current state is GoalsSetupLoaded',
          name: 'GoalsSetupBloc');
      developer.log('🟤 GoalsSetupBloc: Goals valid: ${currentState.isValid}',
          name: 'GoalsSetupBloc');
      developer.log('🟤 GoalsSetupBloc: Goals: ${currentState.goals}',
          name: 'GoalsSetupBloc');

      emit(GoalsSetupSaving());
      developer.log('🟤 GoalsSetupBloc: Emitted GoalsSetupSaving',
          name: 'GoalsSetupBloc');

      try {
        // Сначала пробуем сохранить через UserDataRepository (Supabase)
        developer.log(
            '🟤 GoalsSetupBloc: Trying to save via UserDataRepository',
            name: 'GoalsSetupBloc');
        await _userDataRepository.saveUserGoals(currentState.goals);
        developer.log(
            '🟤 GoalsSetupBloc: Goals saved via UserDataRepository successfully',
            name: 'GoalsSetupBloc');

        // Также сохраняем через use case для совместимости
        developer.log('🟤 GoalsSetupBloc: Calling saveUserGoalsUseCase.execute',
            name: 'GoalsSetupBloc');
        final result = await _saveUserGoalsUseCase.execute(currentState.goals);

        developer.log(
            '🟤 GoalsSetupBloc: SaveUserGoalsUseCase result - isSuccess: ${result.isSuccess}',
            name: 'GoalsSetupBloc');

        if (result.isSuccess) {
          developer.log('🟤 GoalsSetupBloc: Goals saved successfully',
              name: 'GoalsSetupBloc');
          emit(GoalsSetupSaved(currentState.goals));
        } else {
          developer.log(
              '🟤 GoalsSetupBloc: Failed to save goals - error: ${result.error}',
              name: 'GoalsSetupBloc');
          emit(GoalsSetupError(result.error!));
        }
      } catch (e) {
        developer.log('🟤 GoalsSetupBloc: Exception in SaveGoals: $e',
            name: 'GoalsSetupBloc');
        emit(GoalsSetupError('Ошибка сохранения целей: ${e.toString()}'));
      }
    } else {
      developer.log(
          '🟤 GoalsSetupBloc: State is not GoalsSetupLoaded - ${state.runtimeType}',
          name: 'GoalsSetupBloc');
      emit(GoalsSetupError('Неверное состояние для сохранения целей'));
    }
  }

  void _onLoadExistingGoals(
      LoadExistingGoals event, Emitter<GoalsSetupState> emit) async {
    developer.log('🟤 GoalsSetupBloc: LoadExistingGoals event received',
        name: 'GoalsSetupBloc');
    emit(GoalsSetupLoading());

    try {
      // Сначала пробуем загрузить через UserDataRepository (Supabase)
      developer.log('🟤 GoalsSetupBloc: Trying to load via UserDataRepository',
          name: 'GoalsSetupBloc');
      final goals = await _userDataRepository.getUserGoals();

      if (goals != null) {
        developer.log(
            '🟤 GoalsSetupBloc: Goals loaded via UserDataRepository successfully',
            name: 'GoalsSetupBloc');
        final int safeCalories = (goals.targetCalories != null &&
                goals.targetCalories! >= 800 &&
                goals.targetCalories! <= 5000)
      ...
      // Fallback к use case
      developer.log(
          '🟤 GoalsSetupBloc: No goals from UserDataRepository, trying use case',
          name: 'GoalsSetupBloc');
      final result = await _getUserGoalsUseCase.execute();

      if (result.isSuccess && result.goals != null) {
        developer.log(
            '🟤 GoalsSetupBloc: Goals loaded via use case successfully',
            name: 'GoalsSetupBloc');
        final int safeCalories = (result.goals!.targetCalories != null &&
                result.goals!.targetCalories! >= 800 &&
                result.goals!.targetCalories! <= 5000)
        ...
        developer.log(
            '🟤 GoalsSetupBloc: Error loading goals via use case: ${result.error}',
            name: 'GoalsSetupBloc');
        emit(GoalsSetupError(result.error!));
      } else {
        // Нет существующих целей, создаем новые
        developer.log('🟤 GoalsSetupBloc: No existing goals, creating new ones',
            name: 'GoalsSetupBloc');
        emit(GoalsSetupLoaded(
          goals: UserGoals(
            id: 'goals10',
      ...
      developer.log('🟤 GoalsSetupBloc: Exception in LoadExistingGoals: $e',
          name: 'GoalsSetupBloc');
      emit(GoalsSetupError('Ошибка загрузки целей: ${e.toString()}'));
    }
  }

  void _onAllergenToggled(
      AllergenToggled event, Emitter<GoalsSetupState> emit) {
    if (state is GoalsSetupLoaded) {
      final currentState = state as GoalsSetupLoaded;
      ...
      final updatedGoals = currentState.goals
          .copyWith(healthConditions: allergens, updatedAt: DateTime.now());
      emit(currentState.copyWith(
          goals: updatedGoals, isValid: updatedGoals.isValid));
    }
  }

  void _onWorkoutTypeToggled(
      WorkoutTypeToggled event, Emitter<GoalsSetupState> emit) {
    if (state is GoalsSetupLoaded) {
      final currentState = state as GoalsSetupLoaded;
      ...
      final updatedGoals = currentState.goals
          .copyWith(workoutTypes: types, updatedAt: DateTime.now());
      emit(currentState.copyWith(
          goals: updatedGoals, isValid: updatedGoals.isValid));
    }
  }

  void _onWorkoutFrequencyChanged(
      WorkoutFrequencyChanged event, Emitter<GoalsSetupState> emit) {
    if (state is GoalsSetupLoaded) {
      final currentState = state as GoalsSetupLoaded;
      final updatedGoals = currentState.goals.copyWith(
          workoutFrequency: event.frequency, updatedAt: DateTime.now());
      emit(currentState.copyWith(
          goals: updatedGoals, isValid: updatedGoals.isValid));
    }
  }

  void _onWorkoutDurationChanged(
      WorkoutDurationChanged event, Emitter<GoalsSetupState> emit) {
    if (state is GoalsSetupLoaded) {
      final currentState = state as GoalsSetupLoaded;
      final updatedGoals = currentState.goals.copyWith(
          targetProtein: event.duration.toDouble(), updatedAt: DateTime.now());
      emit(currentState.copyWith(
          goals: updatedGoals, isValid: updatedGoals.isValid));
    }
  }

  void _onInitializeGoals(
      InitializeGoals event, Emitter<GoalsSetupState> emit) async {
    developer.log('🟤 GoalsSetupBloc: InitializeGoals event received',
        name: 'GoalsSetupBloc');
