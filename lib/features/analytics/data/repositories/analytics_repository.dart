...
import 'package:nutry_flow/features/analytics/domain/entities/analytics_event.dart'
    as domain;
...
class AnalyticsRepository {
  ...
  Future<void> trackEvent(domain.AnalyticsEvent event) async {
    try {
      developer.log('📊 AnalyticsRepository: Tracking event: ${event.name}',
      ...
      if (_supabaseService.isAvailable) {
        ...
        if (user == null) {
          developer.log(
              '📊 AnalyticsRepository: User not authenticated for tracking',
              name: 'AnalyticsRepository');
          return;
        }
        ...
        developer.log('📊 AnalyticsRepository: Event tracked successfully',
            name: 'AnalyticsRepository');
      } else {
        developer.log(
            '📊 AnalyticsRepository: Supabase not available, event tracking skipped',
            name: 'AnalyticsRepository');
        // В демо-режиме просто логируем событие
        developer.log(
            '📊 AnalyticsRepository: Demo mode - Event: ${event.name}, Parameters: ${event.parameters}',
            name: 'AnalyticsRepository');
      }
    } catch (e) {
      developer.log('📊 AnalyticsRepository: Track event failed: $e',
          name: 'AnalyticsRepository');
  ...
  Future<void> saveNutritionTracking(NutritionTracking tracking) async {
    try {
      if (_supabaseService.isAvailable) {
        developer.log(
            '📊 AnalyticsRepository: Saving nutrition tracking via Supabase',
            name: 'AnalyticsRepository');
        ...
        developer.log(
            '📊 AnalyticsRepository: Nutrition tracking saved successfully',
            name: 'AnalyticsRepository');
      } else {
        developer.log(
            '📊 AnalyticsRepository: Supabase not available, using mock',
            name: 'AnalyticsRepository');
        await _saveNutritionTrackingLocally(tracking);
      }
    } catch (e) {
      developer.log(
          '📊 AnalyticsRepository: Save nutrition tracking failed: $e',
          name: 'AnalyticsRepository');
      rethrow;
    }
  }
  ...
  Future<List<NutritionTracking>> getNutritionTracking(
      DateTime startDate, DateTime endDate) async {
    try {
      if (_supabaseService.isAvailable) {
        developer.log(
            '📊 AnalyticsRepository: Getting nutrition tracking via Supabase',
            name: 'AnalyticsRepository');

        final user = _supabaseService.currentUser;
        if (user == null) {
          developer.log('📊 AnalyticsRepository: User not authenticated',
              name: 'AnalyticsRepository');
          return [];
        }

        final data = await _supabaseService.getUserData('nutrition_tracking',
            userId: user.id);

        final tracking = data
            .map((item) => NutritionTracking(
                  id: item['id'],
                  userId: user.id,
        ...
        // Фильтруем по датам
        final filteredTracking = tracking
            .where((item) =>
                item.date
        ...
        developer.log(
            '📊 AnalyticsRepository: Nutrition tracking retrieved successfully',
            name: 'AnalyticsRepository');
      ...
      developer.log('📊 AnalyticsRepository: Get nutrition tracking failed: $e',
          name: 'AnalyticsRepository');
      rethrow;
    }
  }
  ...
  Future<void> saveWeightTracking(WeightTracking tracking) async {
    try {
      if (_supabaseService.isAvailable) {
        developer.log(
            '📊 AnalyticsRepository: Saving weight tracking via Supabase',
            name: 'AnalyticsRepository');
        ...
        developer.log(
            '📊 AnalyticsRepository: Weight tracking saved successfully',
            name: 'AnalyticsRepository');
      } else {
        developer.log(
            '📊 AnalyticsRepository: Supabase not available, using mock',
      ...
      developer.log('📊 AnalyticsRepository: Save weight tracking failed: $e',
          name: 'AnalyticsRepository');
      rethrow;
    }
  }
  ...
  Future<List<WeightTracking>> getWeightTracking(
      DateTime startDate, DateTime endDate) async {
    try {
      if (_supabaseService.isAvailable) {
        developer.log(
            '📊 AnalyticsRepository: Getting weight tracking via Supabase',
            name: 'AnalyticsRepository');
        ...
        final data = await _supabaseService.getUserData('weight_tracking',
            userId: user.id);

        final tracking = data
            .map((item) => WeightTracking(
                  id: item['id'],
                  userId: user.id,
                  date: DateTime.parse(item['date']),
        ...
        developer.log(
            '📊 AnalyticsRepository: Weight tracking retrieved successfully',
            name: 'AnalyticsRepository');
        return filteredTracking;
      } else {
        developer.log(
      ...
      developer.log('📊 AnalyticsRepository: Get weight tracking failed: $e',
          name: 'AnalyticsRepository');
      rethrow;
    }
  }
  ...
  Future<void> saveActivityTracking(ActivityTracking tracking) async {
    try {
      if (_supabaseService.isAvailable) {
        developer.log(
            '📊 AnalyticsRepository: Saving activity tracking via Supabase',
            name: 'AnalyticsRepository');
        ...
        developer.log(
            '📊 AnalyticsRepository: Activity tracking saved successfully',
            name: 'AnalyticsRepository');
      } else {
        developer.log(
            '📊 AnalyticsRepository: Supabase not available, using mock',
      ...
      developer.log('📊 AnalyticsRepository: Save activity tracking failed: $e',
          name: 'AnalyticsRepository');
      rethrow;
    }
  }
  ...
  Future<List<ActivityTracking>> getActivityTracking(
      DateTime startDate, DateTime endDate) async {
    try {
      if (_supabaseService.isAvailable) {
        developer.log(
            '📊 AnalyticsRepository: Getting activity tracking via Supabase',
            name: 'AnalyticsRepository');
        ...
        final data = await _supabaseService.getUserData('activity_tracking',
            userId: user.id);

        final tracking = data
            .map((item) => ActivityTracking(
                  id: item['id'],
                  userId: user.id,
                  date: DateTime.parse(item['date']),
        ...
        developer.log(
            '📊 AnalyticsRepository: Activity tracking retrieved successfully',
            name: 'AnalyticsRepository');
        return filteredTracking;
      } else {
        developer.log(
      ...
      developer.log('📊 AnalyticsRepository: Get activity tracking failed: $e',
          name: 'AnalyticsRepository');
      rethrow;
    }
  }
  ...
  Future<AnalyticsData> getAnalyticsData(
      DateTime startDate, DateTime endDate) async {
    try {
      developer.log('📊 AnalyticsRepository: Getting analytics data',
          name: 'AnalyticsRepository');
      ...
      developer.log(
          '📊 AnalyticsRepository: Analytics data retrieved successfully',
          name: 'AnalyticsRepository');
      return analyticsData;
    } catch (e) {
      developer.log('📊 AnalyticsRepository: Get analytics data failed: $e',
          name: 'AnalyticsRepository');
      rethrow;
    }
  }
  ...
  Future<void> _saveNutritionTrackingLocally(NutritionTracking tracking) async {
    ...
    developer.log('📊 AnalyticsRepository: Mock save nutrition tracking',
        name: 'AnalyticsRepository');
  }

  Future<List<NutritionTracking>> _getNutritionTrackingLocally(
      DateTime startDate, DateTime endDate) async {
    // TODO: Реализовать локальное получение через SharedPreferences
    developer.log('📊 AnalyticsRepository: Mock get nutrition tracking',
        name: 'AnalyticsRepository');
    return [];
  }
  ...
  Future<void> _saveWeightTrackingLocally(WeightTracking tracking) async {
    // TODO: Реализовать локальное сохранение через SharedPreferences
    developer.log('📊 AnalyticsRepository: Mock save weight tracking',
        name: 'AnalyticsRepository');
  }

  Future<List<WeightTracking>> _getWeightTrackingLocally(
      DateTime startDate, DateTime endDate) async {
    // TODO: Реализовать локальное получение через SharedPreferences
    developer.log('📊 AnalyticsRepository: Mock get weight tracking',
        name: 'AnalyticsRepository');
    return [];
  }
  ...
  Future<void> _saveActivityTrackingLocally(ActivityTracking tracking) async {
    // TODO: Реализовать локальное сохранение через SharedPreferences
    developer.log('📊 AnalyticsRepository: Mock save activity tracking',
        name: 'AnalyticsRepository');
  }

  Future<List<ActivityTracking>> _getActivityTrackingLocally(
      DateTime startDate, DateTime endDate) async {
    // TODO: Реализовать локальное получение через SharedPreferences
    developer.log('📊 AnalyticsRepository: Mock get activity tracking',
        name: 'AnalyticsRepository');
    return [];
  }
}
