...
class CalendarRepositoryImpl implements CalendarRepository {
  ...
  Future<List<CalendarEvent>> getAllEvents() async {
    try {
      ...
      final events = response.map((data) => _mapToCalendarEvent(data)).toList();
  ...
  Future<List<CalendarEvent>> getEventsByDateRange(DateTime startDate, DateTime endDate) async {
    try {
      ...
      return allEvents.where((event) =>
        event.dateTime.isAfter(startDate.subtract(const Duration(days: 1))) &&
  ...
  @override
  Future<CalendarEvent> markEventAsCompleted(String id, bool isCompleted) async {
    try {
      final response = await _supabaseService.updateData(
        _tableName,
        {'is_completed': isCompleted, 'updated_at': DateTime.now().toIso8601String()},
        'id',
        id,
      );
