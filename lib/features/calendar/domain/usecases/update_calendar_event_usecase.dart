...
class UpdateCalendarEventUseCase {
  ...
  Future<UpdateCalendarEventResult> execute(CalendarEvent event) async {
    try {
      ...
      if (existingEvent == null) {
        return const UpdateCalendarEventResult.failure('Событие не найдено');
      }
  ...
  Future<UpdateCalendarEventResult> markAsCompleted(
      String eventId, bool isCompleted) async {
    try {
      // Проверяем аутентификацию
      final currentUser = await _authRepository.getCurrentUser();
      if (currentUser == null) {
      ...
      // Отмечаем событие
      final updatedEvent =
          await _repository.markEventAsCompleted(eventId, isCompleted);
