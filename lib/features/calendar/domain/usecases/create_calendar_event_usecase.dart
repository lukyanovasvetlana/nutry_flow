...
class CreateCalendarEventUseCase {
  ...
  String? _validateEvent(CalendarEvent event) {
    ...
    if (event.endDateTime != null &&
        event.endDateTime!.isBefore(event.dateTime)) {
  ...
  Future<String?> _checkTimeConflicts(CalendarEvent newEvent) async {
    try {
      ...
      final dayStart = DateTime(newEvent.dateTime.year, newEvent.dateTime.month,
          newEvent.dateTime.day);
      final dayEnd = dayStart.add(const Duration(days: 1));

      final existingEvents =
          await _repository.getEventsByDateRange(dayStart, dayEnd);
  ...
  bool _eventsOverlap(CalendarEvent event1, CalendarEvent event2) {
    ...
    final end1 =
        event1.endDateTime ?? event1.dateTime.add(const Duration(hours: 1));

    final start2 = event2.dateTime;
    final end2 =
        event2.endDateTime ?? event2.dateTime.add(const Duration(hours: 1));
