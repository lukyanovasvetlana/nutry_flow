...
class RecipeService {
  ...
  Future<void> updateRecipe(MenuItem recipe, {List<File>? newPhotos, List<String>? photosToDelete}) async {
    ...
    if (photosToDelete != null && photosToDelete.isNotEmpty) {
      final fileNamesToDelete = photosToDelete.map((url) => url.split('/').last).toList();
      await _client.storage.from('recipes').remove(fileNamesToDelete);
    }

    // 2. Загружаем новые фото
    List<String> updatedPhotoUrls = recipe.photos
        .where((p) => p.url != null && !photosToDelete.toString().contains(p.url!))
        .map((p) => p.url!)
        .toList();
  ...
  Future<void> deleteRecipe(String recipeId) async {
    ...
    final response = await _client.from('recipes').select('photos').eq('id', recipeId).single();
    final List<dynamic> photosData = response['photos'];

    // 2. Удалить каждое фото из Storage
    if (photosData.isNotEmpty) {
      final List<String> photoFileNames = photosData
          .map((photo) {
