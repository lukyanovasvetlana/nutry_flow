...
class ActivityRepositoryImpl implements ActivityRepository {
  ...
  Future<Either<String, ActivitySession>> startActivitySession(
      ActivitySession session) async {
    try {
      final startedSession =
          await _activityService.startActivitySession(session);
  ...
  Future<Either<String, ActivitySession>> updateActivitySession(
    ...
    try {
      final updatedSession =
          await _activityService.updateActivitySession(session);
  ...
  Future<Either<String, ActivitySession>> completeActivitySession(
    ...
    try {
      final completedSession =
          await _activityService.completeActivitySession(sessionId);
  ...
  @override
  Future<Either<String, ActivitySession?>> getCurrentSession(
      String userId) async {
    try {
  ...
  Future<Either<String, List<ActivitySession>>> getUserSessions(String userId,
    ...
    try {
      final sessions =
          await _activityService.getUserSessions(userId, from: from, to: to);
  ...
  @override
  Future<Either<String, ActivityStats>> updateDailyStats(
      ActivityStats stats) async {
    try {
  ...
  Future<Either<String, Map<String, dynamic>>> getActivityAnalytics(
      ...
      DateTime? to}) async {
    try {
      final analytics = await _activityService.getActivityAnalytics(userId,
          from: from, to: to);
  ...
  Future<Either<String, int>> getTotalDuration(String userId,
    ...
    try {
      final duration =
          await _activityService.getTotalDuration(userId, from: from, to: to);
  ...
  Future<Either<String, int>> getTotalCaloriesBurned(String userId,
      {DateTime? from, DateTime? to}) async {
    try {
      final calories = await _activityService.getTotalCaloriesBurned(userId,
          from: from, to: to);
