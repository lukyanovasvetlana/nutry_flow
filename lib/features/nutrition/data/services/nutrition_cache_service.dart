...
class NutritionCacheService {
  ...
  Future<void> cacheFavoriteFoodItems(String userId, List<FoodItemModel> items) async {
    try {
      final key = '${_favoriteFoodItemsKey}_$userId';
      final jsonList = items.map((item) => item.toJson()).toList();
      final jsonString = jsonEncode(jsonList);
  ...
  Future<List<FoodEntryModel>?> getCachedFoodEntries(String userId, DateTime date) async {
    try {
      final key = 'food_entries_${userId}_${date.toIso8601String().split('T')[0]}';
      final jsonString = _prefs.getString(key);
      if (jsonString == null) return null;
  ...
  Future<NutritionSummaryModel?> getCachedNutritionSummary(String userId, DateTime date) async {
    try {
      final key = 'nutrition_summary_${userId}_${date.toIso8601String().split('T')[0]}';
      final jsonString = _prefs.getString(key);
      if (jsonString == null) return null;
  ...
  Future<void> cacheNutritionSummary(String userId, NutritionSummaryModel summary) async {
    try {
      final key = 'nutrition_summary_${userId}_${summary.date.toIso8601String().split('T')[0]}';
  ...
  Future<void> cacheFoodItemByBarcode(String barcode, FoodItemModel item) async {
    try {
      final key = 'barcode_$barcode';
      final jsonString = jsonEncode(item.toJson());
      await _prefs.setString(key, jsonString);
  ...
  Future<Map<String, dynamic>> getCacheStatistics() async {
    try {
      ...
      for (final key in allKeys) {
        if (key.startsWith('food_item_')) foodItemsCount++;
        else if (key.startsWith('food_entries_')) foodEntriesCount++;
        else if (key.startsWith('nutrition_summary_')) summariesCount++;
        else if (key.startsWith('search_suggestions_')) searchSuggestionsCount++;
      }
