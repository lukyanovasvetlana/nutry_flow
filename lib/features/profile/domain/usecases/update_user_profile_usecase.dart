...
class ProfileValidationResult {
  ...
  factory ProfileValidationResult.invalid(List<String> errors) => ProfileValidationResult._(
    isValid: false,
    errors: errors,
  );
}
  ...
  Future<UpdateUserProfileResult> execute(UserProfile profile) async {
    ...
    try {
      ...
      if (!validationResult.isValid) {
        return UpdateUserProfileResult.failure('Validation failed: ${validationResult.errors.join(', ')}');
      }
      
      // Обновляем профиль
      final updatedProfile = await _profileRepository.updateUserProfile(profile);
      
      if (updatedProfile != null) {
        return UpdateUserProfileResult.success(updatedProfile);
      } else {
        return UpdateUserProfileResult.failure('Failed to update profile');
      }
    } catch (e) {
