...
class UserDataRepository {
  ...
  Future<void> saveUserGoals(UserGoals goals) async {
    try {
      developer.log('🟪 UserDataRepository: Saving goals with sync',
      ...
      developer.log('🟪 UserDataRepository: Goals saved successfully',
          name: 'UserDataRepository');
    } catch (e) {
      developer.log('🟪 UserDataRepository: Save goals failed: $e',
          name: 'UserDataRepository');
      rethrow;
    }
  }
  ...
  Future<UserGoals?> getUserGoals() async {
    try {
      developer.log('🟪 UserDataRepository: Getting goals with sync',
          name: 'UserDataRepository');
      ...
      if (_supabaseService.isAvailable) {
        ...
        if (user != null) {
          final data =
              await _supabaseService.getUserData('user_goals', userId: user.id);
      ...
      if (goalsData.isNotEmpty) {
        ...
        final goals = UserGoals(
          ...
          dietaryPreferences:
              List<String>.from(goalData['dietary_preferences'] ?? []),
          healthConditions:
              List<String>.from(goalData['health_conditions'] ?? []),
          ...
          createdAt: DateTime.parse(
              goalData['created_at'] ?? DateTime.now().toIso8601String()),
          updatedAt: DateTime.parse(
              goalData['updated_at'] ?? DateTime.now().toIso8601String()),
        );

        developer.log(
            '🟪 UserDataRepository: Goals retrieved successfully from Supabase',
            name: 'UserDataRepository');
        return goals;
      }

      developer.log('🟪 UserDataRepository: No goals found',
          name: 'UserDataRepository');
      return null;
    } catch (e) {
      developer.log('🟪 UserDataRepository: Get goals failed: $e',
          name: 'UserDataRepository');
      rethrow;
    }
  }
  ...
  Future<void> saveUserProfile(Map<String, dynamic> profile) async {
    try {
      if (_supabaseService.isAvailable) {
        developer.log('🟪 UserDataRepository: Saving profile via Supabase',
            name: 'UserDataRepository');
        ...
        developer.log('🟪 UserDataRepository: Profile saved successfully',
            name: 'UserDataRepository');
      } else {
        developer.log(
            '🟪 UserDataRepository: Supabase not available, using mock',
            name: 'UserDataRepository');
        // Fallback к mock сохранению
        await _saveProfileLocally(profile);
      }
    } catch (e) {
      developer.log('🟪 UserDataRepository: Save profile failed: $e',
          name: 'UserDataRepository');
      rethrow;
    }
  }
  ...
  Future<Map<String, dynamic>?> getUserProfile() async {
    try {
      if (_supabaseService.isAvailable) {
        developer.log('🟪 UserDataRepository: Getting profile via Supabase',
            name: 'UserDataRepository');

        final user = _supabaseService.currentUser;
        if (user == null) {
          developer.log('🟪 UserDataRepository: User not authenticated',
              name: 'UserDataRepository');
          return null;
        }

        final data = await _supabaseService.getUserData('user_profiles',
            userId: user.id);

        if (data.isNotEmpty) {
          developer.log('🟪 UserDataRepository: Profile retrieved successfully',
              name: 'UserDataRepository');
          return data.first;
        }

        developer.log('🟪 UserDataRepository: No profile found',
            name: 'UserDataRepository');
        return null;
      } else {
        developer.log(
      ...
      developer.log('🟪 UserDataRepository: Get profile failed: $e',
          name: 'UserDataRepository');
      rethrow;
    }
  }
